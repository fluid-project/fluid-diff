...
### `gpii.diff.generateTracebackTable(leftArray, rightArray)`
...
Compare two arrays, returning a single longest common sequence (not necessarily contiguous).  When comparing `[1,3,5,7]`
...
If there are multiple sequences of the same length, they are sorted using `gpii.diff.sortByLengthThenTightnessThenIndex`
and only the first match is returned.

### `gpii.diff.longestCommonSequences(leftArray, rightArray)`

* `leftArray` `{Array}` - An array.
* `rightArray` `{Array}` - An array to compare to `leftArray`.
* Returns: `{Array}` - An array containing all "longest" sequences, i. e. different sequences (see above) that are equally long.

Compare two arrays, exactly as with `gpii.diff.longestCommonSequences`, but do not pick a single winner.

### `gpii.diff.tracebackLongestSequences(tracebackTable)`
...
The "traceback" is performed from the lowest right cell.  A pipe (|) indicates that the adjacent upward cell contains
the same number of matches.  An underscore (_) indicates that the adjacent left cell contains the same number of
matches.  A backslash (\) indicates a match in the current cell, and indicates that the rest of the pattern starts in
the upper left diagonal square from ours.  An asterisk (*) represents a terminal point, i.e. there is no match or
inherited material to continue.